...
![ssl]({{ site.url }}/assets/image/server/server_33.png)
...
![ssl]({{ site.url }}/assets/image/server/server_34.png)
...
![ssl]({{ site.url }}/assets/image/server/server_39.png)
...
![ssl]({{ site.url }}/assets/image/server/server_43.png)

# 6.左外连接

`LEFT JOIN`关键字会将左表作为主表(A)，将返回所有行，即使在右表(B)中没有匹配的行会以`NULL`显示。

![ssl]({{ site.url }}/assets/image/server/server_35.png)

```sql
SELECT *
FROM teacher t LEFT JOIN course c on t.t_id = c.t_id
```
![ssl]({{ site.url }}/assets/image/server/server_36.png)

# 7.右外连接

RIGHT JOIN 关键字会右表作为主表(B)，将返回所有行，即使在左表(A)中没有匹配的行会以`NULL`显示。

![ssl]({{ site.url }}/assets/image/server/server_37.png)

```sql
SELECT *
FROM teacher t RIGHT JOIN course c on t.t_id = c.t_id
```
![ssl]({{ site.url }}/assets/image/server/server_38.png)
...
![ssl]({{ site.url }}/assets/image/server/server_42.png)
...
![ssl]({{ site.url }}/assets/image/server/server_41.png)
