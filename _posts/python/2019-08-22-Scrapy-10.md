...
![ssl]({{ site.url }}/assets/image/python/python_54.png)

小菜这里的chrome版本是`78`,所以要找到支持chrome浏览器78版本`ChromeDriver`。然后根据自己的操作系统选择对应的版本

![ssl]({{ site.url }}/assets/image/python/python_55.png)
![ssl]({{ site.url }}/assets/image/python/python_56.png)
...
![ssl]({{ site.url }}/assets/image/python/python_57.gif)

# 7.系统性实现知乎验证

上面只是简单验用`selenium`来模拟一个登陆。真实情况稍微复杂点，下面放一张模拟知乎登陆流程图。后面就整体实现下

![ssl]({{ site.url }}/assets/image/python/python_58.png)
